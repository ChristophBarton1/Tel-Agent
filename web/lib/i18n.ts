...
/** English is the reference shape; `de` and `ar` are checked against it. */
 ...
 *
 * English is the reference: `de` and `ar` are typed against it, so a key added in
 * English and forgotten elsewhere is a build error rather than a blank label.
 */
export function pickDictionary<T>(locale: Locale, dictionaries: { en: T; de: T; ar: T }): T {
  return dictionaries[locale];
}
